...
// theatre layout
  ...
  goToBooking(){
    ...
    this.hs.getUser(email).subscribe((data) => {
      this.hs.selectedDateTimeTheatre={
        ...this.hs.selectedDateTimeTheatre,
        seats: this.cart.selectedSeats,
        numberOfSeat: this.cart.selectedSeats.length,
        amount: this.cart.totalamount,
        userId: data["id"],
        emailId: email
      };

      console.log('POST DATA', this.hs.selectedDateTimeTheatre);
      this.hs.bookSeat(this.hs.selectedDateTimeTheatre).subscribe((data)=>{
        console.log(data+"-----");
      ...
      console.log('Some error occured in getting user id');
    }
    );
    // console.log(this.cart.selectedSeats.length);
